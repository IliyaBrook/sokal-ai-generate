...
export const CollaborationStatus: React.FC<CollaborationStatusProps> = ({
  ...
  const otherWatchers = watchers.filter(
    (watcher) => watcher.userId !== currentUserId
  );
  const currentUser = watchers.find(
    (watcher) => watcher.userId === currentUserId
  );
  const authenticatedWatchers = otherWatchers.filter(
    (watcher) => !watcher.userId.startsWith("anonymous-")
  );
  const anonymousWatchers = otherWatchers.filter((watcher) =>
    watcher.userId.startsWith("anonymous-")
  );
  ...
  return (
    <div className="mb-4 border rounded-md p-3 bg-muted/30">
      ...
      {currentUser && (
        <div className="mt-2 mb-2 max-w-5xl">
          ...
          <Badge variant="default" className="text-xs gap-1">
            ...
            {currentUser.username ||
                (currentUser.userId.startsWith("anonymous-")
